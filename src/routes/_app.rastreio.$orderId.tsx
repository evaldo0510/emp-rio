...
function TrackingPage() {
  ...
  return (
    <div className="container-narrow py-8">
      <Link
        to="/conta"
        className="flex items-center text-sm text-[var(--muted-foreground)] hover:text-[var(--clay)] mb-8 transition-colors"
      >
        ...
        <div className="space-y-8">
          <div className="bg-white rounded-3xl border border-[var(--border)] p-8 shadow-sm">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
              ...
              <div
                className={`px-4 py-2 rounded-full ${currentStatus.bg} ${currentStatus.color} flex items-center gap-2 font-semibold text-sm`}
              >
              ...
              {updates.length === 0 ? (
                <div className="pl-12 py-4">
                  <p className="text-sm text-[var(--muted-foreground)] italic">
                    O histórico de rastreamento ficará disponível assim que o pedido for processado.
                  </p>
                </div>
              ) : (
                updates.map((update, index) => (
                  <div key={update.id} className="relative pl-12 pb-8 last:pb-0">
                    <div
                      className={`absolute left-0 top-1.5 w-[36px] h-[36px] -translate-x-1/2 rounded-full border-4 border-white shadow-sm flex items-center justify-center z-10 ${
                        index === 0
                    ...
                    <div
                      className={
                        index === 0
                          ? "bg-[var(--sand)]/30 p-4 rounded-2xl border border-[var(--border)]"
                          : ""
                      }
                    >
                      <p className="text-xs font-bold text-[var(--muted-foreground)] mb-1">
                        {format(new Date(update.created_at), "dd 'de' MMMM 'às' HH:mm", {
                          locale: ptBR,
                        })}
                      </p>
                      <h3
                        className={`font-semibold ${index === 0 ? "text-[var(--clay)] text-base" : "text-[var(--coffee)] text-sm"}`}
                      >
                        {update.description}
                      </h3>
                      {update.location && (
        ...
        <aside className="space-y-6">
          ...
          {shipment?.carrier && (
            <div className="bg-[var(--sand)]/50 rounded-3xl border border-[var(--border)] p-6 shadow-sm">
              <h3 className="text-sm font-bold uppercase tracking-widest text-[var(--muted-foreground)] mb-3">
                Transportadora
              </h3>
              ...
              {shipment.carrier === "Melhor Envio" && (
                <>
                  ...
                  <Button asChild variant="soft" size="sm" className="w-full">
                    <a
                      href="https://melhorenvio.com.br/rastreamento"
                      target="_blank"
