    ...
    FOR v_order IN SELECT id FROM public.orders WHERE status = 'paid' LOOP
        FOR v_item IN SELECT * FROM public.order_items WHERE order_id = v_order.id LOOP
            IF v_item.seller_id IS NOT NULL THEN
                ...
                INSERT INTO public.wallet_transactions (wallet_id, order_id, amount, commission, type, description)
                VALUES (v_wallet_id, v_order.id, v_net_amount, v_commission_amount, 'sale', 'Venda do item: ' || COALESCE(v_item.name, 'Produto'));
            END IF;
        END LOOP;
    END LOOP;
